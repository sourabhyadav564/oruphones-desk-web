...
const Pricerange = () => {
  ...
  useEffect(() => {
   const fetchData = async ()=>{
     const priceRange = await Axios.shopByPriceRange((max === "above"?"200000":max),getSearchLocation,min,Cookies.get("userUniqueId") || "Guest");
      ...
      {!isLoading && bestDeal && bestDeal.length > 0 && (
          <Carousel {...settings}>
            {bestDeal.map((items,index) => (
              <BestDealsCard key={index} data={items} setProducts={setBestDeal} className = "bestDealCarousel"/>
            ))}
          </Carousel>
        )}
        <div className="grid grid-cols-3 gap-4 mt-3">
          {!isLoading && sortingProducts && sortingProducts.length > 0?(sortingProducts?.map((product, index) => (
            <ProductCard key={index} data={product} prodLink setProducts={setOtherListings}/>
          ))):(
