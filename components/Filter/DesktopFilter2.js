...
const DesktopFilter = ({ setFilters, filterOptions }) => {
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			brand: selectedBrand?.filter((val) => val !== 'all'),
		}));
	}, [selectedBrand]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			condition: selectedCondition?.filter((val) => val !== 'all'),
		}));
	}, [selectedCondition]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			color: selectedColor?.filter((val) => val !== 'all'),
		}));
	}, [selectedColor]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			Ram: selectedRam?.filter((val) => val !== 'all'),
		}));
	}, [selectedRam]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			storage: selectedStorage?.filter((val) => val !== 'all'),
		}));
	}, [selectedStorage]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			warranty: selectedWarranty?.filter((val) => val !== 'all'),
		}));
	}, [selectedWarranty]);
	...
	useEffect(() => {
		setFilters((prev) => ({
			...prev,
			verification: selectedVerification?.includes('all')
				? []
