...
function ArticleCard({ title, src, href, viewAll }) {
  if (viewAll) {
    ...
    return (
      ...
      <Link href={`${common}`}>
        <a className="w-full h-full rounded-md shadow hover:shadow-md flex flex-col justify-center items-center text-m-green bg-m-white" target="_blank">
          View All
        </a>
      </Link>
      // </Link>
  ...
  return (
    // <Link href={`#blogs${href}`}>
    <Link href={`${href}`} >
      <a className="w-full h-full rounded-md shadow hover:shadow-md flex flex-col items-center bg-m-white" target="_blank">
